    ...
    try {
      if (channelName.length > 0 && selectedContacts.length > 0) {
        ...
        if (response.status === 200) {
          setChannelName("");
          setSelectedContacts([]);
          setNewChannelModal(false); // Close the dialog
          addChannel(response.data.channel); // Add the new channel to state

          // Toast success message
          toast.success(
            `Channel "${response.data.channel.name}" created successfully!`
          );
        }
      } else {
        // Validation feedback
        toast.error("Channel name and at least one contact are required.");
      }
    } catch (error) {
